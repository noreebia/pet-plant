...
for img in glob.glob("C://Project/my-pet-plant/image_classification_module/data/train/"+ speices +"/*.jpg"):
...
for img in glob.glob("C://Project/my-pet-plant/image_classification_module/data/validation/"+ speices +"/*.jpg"):
    cv_img = cv2.imread(img, cv2.IMREAD_GRAYSCALE)
    try:
        blur = cv2.GaussianBlur(cv_img, (5, 5), 0)
        ret, binary = cv2.threshold(blur, 0, 255, cv2.THRESH_BINARY+cv2.THRESH_OTSU)
